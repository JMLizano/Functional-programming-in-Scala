...
object Interaction2 {
  ...
  def availableLayers: Seq[Layer] = {
    val avgTempLayer = Layer(LayerName.Temperatures, temperatures, 1975 to 2015)
    val devTempLayer = Layer(LayerName.Deviations, deviations, 1975 to 2015)
    Seq(avgTempLayer, devTempLayer)
  }
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
    Signal(math.max(selectedLayer().bounds.head, math.min(selectedLayer().bounds.last, sliderValue())))
  }
  ...
  def layerUrlPattern(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] = {
    ...
    Signal(s"target/$name/$selectedYear()")
  }
