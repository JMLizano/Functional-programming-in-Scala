...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((z,a) => if (a.mentionsLanguage(lang)) z + 1 else z, _ + _)
  }
...
//  }
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(a => langs.map(l => a.mentionsLanguage(l) match { case true => (l, a) case false => null }))
      .filter(_ != null)
      .groupByKey()
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd
      .flatMap(a => langs.map(l => (l, a.mentionsLanguage(l) match { case true => 1 case false => 0 })))
      .reduceByKey(_ + _)
      .collect()
      .toList
      .sortBy(_._2)
