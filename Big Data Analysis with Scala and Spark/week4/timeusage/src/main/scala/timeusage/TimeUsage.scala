...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val first = Seq(StructField(columnNames.head, StringType, false))

    StructType(first ++ columnNames.tail.map(StructField(_, DoubleType, false)))
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def prefixMatch(name: String, prefix: List[String]) : Boolean = prefix.exists(name.startsWith(_))

    val primaryNeeds = columnNames.filter(prefixMatch(_, List("t01", "t03", "t11", "t1801", "t1803")))
    val working      = columnNames.filter(prefixMatch(_, List("t05", "t1805")))
    val leisure      = columnNames.filter(prefixMatch(_, List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")))

    (
      primaryNeeds.map(col),
      working.map(col),
      leisure.filter(c => !working.contains(c) && !primaryNeeds.contains(c)).map(col)
    )
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = functions.when(df("telfs") >= 1 && df("telfs") < 3, "working")
                                                     ...
                                                     .as("working")
    val sexProjection: Column = functions.when(df("tesex") === 1, "male")
                                           .otherwise("female")
                                           .as("sex")
