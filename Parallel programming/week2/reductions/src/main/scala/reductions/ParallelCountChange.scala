...
//    val coins = List(1, 2, 5, 10)
    ...
    if (args.length > 0 && args(0) == "myThreshold")
...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (money <= 0 || coins.isEmpty || threshold(money, coins))
      ...
      val(tl, tr) = parallel(parCountChange(money, coins.tail, threshold),
  ...
  def myThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
