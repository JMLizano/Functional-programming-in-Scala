...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val mapMeansPoints = points.groupBy(findClosest(_, means))
    // Add means without assigned points
    means.map(x => (x, mapMeansPoints.getOrElse(x, GenSeq[Point]()))).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    !oldMeans.zip(newMeans).map(x => x._1.squareDistance(x._2)).exists(_ > eta)
  }
